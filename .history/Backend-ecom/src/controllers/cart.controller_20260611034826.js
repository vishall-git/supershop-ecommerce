...
async function deleteCartItems(req, res) {
  ...
  try {
    ...
    if (!itemExist) { return res.status(404).json({ message: "not found" }) }
  ...
  } catch (err) { return res.status(500).json({ message: "internal sever error" }) }
}

  module.exports = { createCart,deleteCartItems }
